	...
	let disposable = vscode.commands.registerTextEditorCommand('extension.restcall', function (editor) {
		...
		if (lineContent !== '') {
			...
			if (HTTPRequest.length >= 3) { // Content atleast have HTTP method and URI `// {HTTP_METHOD} {URI}`
				...
				if (index !== -1) {
					vscode.workspace.openTextDocument(`${__dirname}/Response.json`)
						.then(doc =>  {
							vscode.window.showTextDocument(doc, { viewColumn: vscode.ViewColumn.Beside })
							return axios(configObj);
						})
						.then(resp => {
							let json = givenOnlyKey ? util.inspect(resp[givenOnlyKey]) : util.inspect(resp);
							fs.writeFileSync(`${__dirname}/Response.json`, json);
						});
				} else {
					vscode.window.showErrorMessage(`Please select valid HTTP Method ['GET', 'POST', 'PUT', 'DELETE']`);
				}
			} else {
				vscode.window.showErrorMessage('Content atleast have HTTP Method and URI `// {HTTP_METHOD} {URI}`');
			}
		} else {
